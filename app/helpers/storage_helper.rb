module StorageHelper
    def getStorage_by_id(id)
        ...
        if @store.nil?
            ...
            return {"id": id, "data": data, "meta": meta, "dri": @store.dri, "created-at": @store.created_at, "updated-at": @store.updated_at, "schema": @store.schema}
        end
    end
    ...
    def newStorage(col_id, data, meta, dri, key)
        ...
        schema = meta["schema"] rescue nil
        @store = Store.new(dri: dri, key: key, schema: schema)
        @store.save
        id = @store.id
