class UsersController < ApplicationController
    ...
    def create
        data = params.except(:controller, :action, :user, "controller", "action", "user")
    ...
    def read
        ...
        render json: retVal.merge({"user-id" => @store.id, "organization-id" => org_id}),
               status: 200
    end
    ...
    def wallet
        ...
        if @dk.length > 0
            @dk = @dk.first
            retVal["oauth"] = {"client-id": @dk.uid, "client-secret": @dk.secret, "scopes": @dk.scopes}
        end
        if !meta["dlt"].nil?
            retVal["dlt"] = meta["dlt"]
        end
