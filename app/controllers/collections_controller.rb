class CollectionsController < ApplicationController
    ...
    before_action -> { doorkeeper_authorize! :admin }, only: [:get_access, :purge]
    ...
    def objects
        ...
        @obj = Store.where(key: "object_" + id.to_s).order(created_at: :desc)
        page = params[:page] || 1
        if page == "all"
            page = 1
            items = @obj.count
        ...
        @pagy, @records = pagy_array(@obj, page: page, limit: items)
    ...
    def purge
        ...
        @objs = Store.where(key: "object_" + id.to_s)
        count = @objs.count.dup
        ...
        @objs.each do |item|
          begin
            ...
            @errors << { id: item.id, exception: e.message }
          end
        end
        if errors.count > 0
